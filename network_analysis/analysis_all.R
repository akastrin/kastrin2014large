...
df <- read.table('uid2uid_all.txt', header = FALSE)
df <- df[, 1:2]
...
# Weights in the 3rd field
df <- read.table('uid2uid_all_ct.txt', header = FALSE)
df <- df[, 1:2]
g <- graph.data.frame(df)
g <- as.undirected(g)
g <- simplify(g)
...
df <- read.table('network_all.txt', header = FALSE)
g <- graph.data.frame(df)
g <- as.undirected(g)
g <- simplify(g)
vcount(g)
...
write.table(g.giant.edges, file = 'giant_all.txt', col.names = FALSE, row.names = FALSE, quote = FALSE)

# Network analysis on giant component
df <- read.table('giant_all.txt', header=F)
g <- graph.data.frame(df)
g <- as.undirected(g)
g <- simplify(g)
vcount(g)
...
write.table(cent.df, file = "centrality_measures_all.txt", row.name = FALSE, quote = FALSE)

# Try to fit power-law model
cent.df <- read.table(file = "centrality_measures_all.txt", header = TRUE)
deg <- cent.df$degree
